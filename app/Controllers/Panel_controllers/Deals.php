...
class Deals extends BaseController {
    ...
    public function __construct() {
        ...
        if(!Permissions::is_role_allowed(DEALS_TASK, (isset($session->id_rol) ? $session->id_rol : 0))){
    ...
    public function index() {
        if($this->is_allowed){
            return $this->create_view('panel_views/deals', $this->load_data());
        }//end if not allowed
        else {
    ...
    private function load_data() {
        ...
        $this->breadcrumb->add_breadcrumb('Ofertas', 'panel/ofertas');
    ...
    private function create_view($view_name = '', $content = array()){
        $content['menu'] = generate_nav_menu(DEALS_TASK, session()->id_rol);
