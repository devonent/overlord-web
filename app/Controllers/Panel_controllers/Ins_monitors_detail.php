...
class Ins_monitors_detail extends BaseController {
    ...
    public function __construct() {
        ...
        if(!Permissions::is_role_allowed(INS_MONITORS_DETAIL_TASK, (isset($session->id_rol) ? $session->id_rol : 0))){
            $this->is_allowed = FALSE;
        }//end if role not allowed
    }//end __construct function

    public function index($id_monitor = 0) {
        $monitor_table = new \App\Models\Tabla_monitor();
        if($this->is_allowed){
            if($monitor_table->find($id_monitor) != NULL){
                return $this->create_view('panel_views/ins_monitors_detail', $this->load_data($id_monitor));
            }// end if existe bateria
            else{
                create_user_message('El monitor a editar no existe...', 'error');
    ...
    private function load_data($id_monitor = 0) {
        $data = array();
        //Session elements
        $session = session();
        ...
        $data['section_name'] = 'Editar monitor';

        // Obtener datos de la batería
        $monitor_table = new \App\Models\Tabla_monitor();
        $monitor_details = $monitor_table->get_single_monitor($id_monitor);
        $data['monitor_details'] = $monitor_details;
        ...
        $this->breadcrumb->add_breadcrumb('Editar monitor: <b>'. $monitor_details->marca . ' ' . $monitor_details->modelo . ' ' . $monitor_details->acabado_color . '</b>', 'panel/monitores');
    ...
    private function create_view($view_name = '', $content = array()){
        $content['menu'] = generate_nav_menu(INS_MONITORS_DETAIL_TASK, session()->id_rol);
        return view($view_name, $content);
    }//end create_view function

    public function update_monitor(){
        $id_monitor = $this->request->getPost('id_monitor');
        $monitor_table = new \App\Models\Tabla_monitor();
        if($monitor_table->find($id_monitor) != NULL){
            ...
            if(($this->request->getFile('imagen-producto'))->getSize() > 0) {
                ...
                $monitor['imagen'] = $monitor_in_db->imagen;
            }// else si hay imagen insertada

            if(($monitor_table->update($id_monitor, $monitor)) > 0){
                create_user_message('El monitor se actualizó con éxito', 'success');
                return redirect()->to(route_to('panel/monitores/editar_monitor/', $id_monitor));
            }// end if se inserta monitor
            else {
                create_user_message('Hubo un problema al actualizar el monitor. Intenta de nuevo', 'error');
                return redirect()->to(route_to('panel/monitores/editar_monitor/', $id_monitor));
